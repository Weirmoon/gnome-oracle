...
type Turn = { character: Character; text: string; reasoning?: string; phase: string; historyId?: number; error?: string; complete?: boolean };
...
export default function Council({ characters, resumeId }: { characters: Character[]; resumeId?: number }) {
  ...
  const [cid, setCid] = useState<number | undefined>(resumeId), [busy, setBusy] = useState(false), [error, setError] = useState(""), [serious, setSerious] = useState(false), [sources, setSources] = useState<SearchSource[]>([]);
  ...
  async function run(retryIndex?: number) {
    ...
    if (retryIndex === undefined) { setTurns({}); setCid(undefined); setSources([]); }
    const current: Record<number, Turn> = retryIndex === undefined ? {} : { ...turns };
    try {
      const res = await apiFetch("/api/council", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify(retryIndex === undefined ? { characterIds: ids, question, serious } : { consultationId: cid, retryIndex, serious }), signal: abort.current.signal });
      ...
      for (;;) {
        ...
        while ((nl = buffer.indexOf("\n")) >= 0) {
          ...
          if (event.type === "thinking") current[event.index].reasoning = (current[event.index].reasoning ?? "") + event.text;
  ...
  return <section className="panel council"><h2>Oracle Council</h2><p className="persona-desc">Three perspectives, then three rebuttals. Each turn uses your active AI connection.</p>
    ...
    <label className="field">Question<textarea value={question} disabled={busy} onChange={e => setQuestion(e.target.value)} /></label><label><input type="checkbox" checked={serious} disabled={busy} onChange={e => setSerious(e.target.checked)} /> 🎓 Serious mode (accuracy first, slower)</label><div className="row"><button onClick={() => run()} disabled={busy || !question.trim() || new Set(ids).size !== 3}>{busy ? "Council in session…" : "Convene Council"}</button><button className="ghost" onClick={() => { abort.current?.abort(); voiceEpoch.current++; tts.cancel(); }} disabled={!busy}>Stop</button></div>
    <Sources sources={sources} />
    {Object.entries(turns).map(([key, t]) => <article className="histcard panel" key={key}><b>{t.character.emoji} {t.character.name} · {t.phase}</b><Pondering reasoning={t.reasoning ?? ""} pondering={busy && !t.text && !t.complete} /><p className="hista">{t.text || (t.error ? "" : "Thinking…")}</p>{t.error && <p className="error">{t.error}</p>}
