...
export default function Consultation({ id }: { id: number }) {
  const [consultation, setConsultation] = useState<any>(null), [question, setQuestion] = useState(""), [answer, setAnswer] = useState(""), [reasoning, setReasoning] = useState(""), [serious, setSerious] = useState(false), [sources, setSources] = useState<SearchSource[]>([]), [busy, setBusy] = useState(false), [error, setError] = useState("");
  ...
  async function ask() {
    ...
    abort.current?.abort(); const controller = new AbortController(); abort.current = controller; setBusy(true); setAnswer(""); setReasoning(""); setSources([]); setError(""); tts.begin();
    try {
      const res = await apiFetch("/api/ask", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ question, consultationId: id, serious }), signal: controller.signal });
      if (!res.ok) throw new Error((await res.json()).error); setSources(decodeSources(res.headers.get(SOURCES_HEADER))); const reader = res.body!.getReader(), decoder = new TextDecoder(), splitter = new ReasoningSplitter();
      for (;;) { const { done, value } = await reader.read(); if (done) break; const { answer: text, reasoning: thought } = splitter.push(decoder.decode(value, { stream: true })); if (thought) setReasoning(old => old + thought); if (text) { setAnswer(old => old + text); tts.feed(text); } }
  ...
  return <main className="wrap"><div className="topbar"><h1 className="title">Consultation #{id} <span className="spark">🔮</span></h1><Link className="navlink" href="/history">← History</Link></div><p className="tagline">Resuming {character?.emoji} {character?.name ?? "the oracle"}. Only this consultation is remembered.</p>
    ...
    {answer && <div className="bubble">{answer}</div>}{busy && <Pondering reasoning={reasoning} pondering={!answer} />}<Sources sources={sources} /><div className="row"><input value={question} onChange={e => setQuestion(e.target.value)} onKeyDown={e => { if (e.key === "Enter") void ask(); }} placeholder="Continue this consultation…" /><button disabled={busy || !question.trim()} onClick={() => void ask()}>{busy ? "Conjuring…" : "Ask again"}</button></div><label><input type="checkbox" checked={serious} onChange={e => setSerious(e.target.checked)} /> 🎓 Serious mode (accuracy first, slower)</label>{error && <p role="alert">{error}</p>}</main>;
}
